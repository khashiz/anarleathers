...
?>
<a href="javascript:void(0)" class="uk-padding-small uk-padding-remove-horizontal uk-display-inline-block uk-text-primary uk-border-rounded hoverIcon"><img src="<?php echo JUri::base().'images/sprite.svg#globe' ?>" alt="" width="24" height="24" data-uk-svg></a>
<div data-uk-drop="pos: bottom-left; offset: 0;">
    <div class="uk-border-rounded uk-box-shadow-small uk-background-white langSwitcher <?php echo $moduleclass_sfx; ?>">
        <?php if ($headerText) : ?>
        ...
        <?php else : ?>
            <ul class="uk-flex uk-text-zero uk-padding-small uk-margin-remove" dir="<?php echo JFactory::getLanguage()->isRtl() ? 'rtl' : 'ltr'; ?>">
                <?php foreach ($list as $language) : ?>
                    <?php if (!$language->active) : ?>
                        <li>
                            <a class="uk-flex uk-flex-center uk-flex-middle uk-flex-column uk-border-rounded" href="<?php echo htmlspecialchars_decode(htmlspecialchars($language->link, ENT_QUOTES, 'UTF-8'), ENT_NOQUOTES); ?>">
                                <?php if ($params->get('image', 1)) : ?>
                                    ...
                                    <?php endif; ?>
                                    <span class="uk-display-block uk-text-tiny uk-text-center font uk-margin-small-top"><?php echo $params->get('full_name', 1) ? $language->title_native : strtoupper($language->sef); ?></span>
                                <?php else : ?>
