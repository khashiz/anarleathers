    ...
    <div class="uk-grid-column-large uk-grid-row-medium" data-uk-grid>
        <div class="uk-width-1-1 uk-width-auto@m">
            ...
            if($this->invoice_type == 'order' || empty($this->element->order_invoice_number)) {
                echo '<h3 class="uk-text-primary uk-margin-bottom sectionTitle font">'.JText::sprintf('HIKASHOP_ORDER_TITLE').'</h3>';
            } else {
                echo '<h3 class="uk-text-primary uk-margin-bottom sectionTitle font">'.JText::_(strtoupper($this->invoice_type)).': '.@$this->element->order_invoice_number.'</h3>';
            }
            ?>
            <?php
            $status = "";
            ...
            ?>
            <div class="uk-text-small uk-text-secondary font">
                <?php echo '<span class="uk-text-small uk-text-secondary f600 font">'.JText::sprintf('ORDER_ROW_CODE', @$this->element->order_number).'</span><br>'; ?>
                <?php echo '<span class="uk-text-muted">'.JText::sprintf('ORDER_ROW_STATUS').'</span><span class="'.$status.'">'.JText::sprintf('ORDER_STATUS_'.@$this->element->order_status).'</span><br>'; ?>
                <!-- DATE -->
                <?php
                if($this->invoice_type == 'order' || empty($this->element->order_invoice_created)) {
                    echo '<span class="uk-text-muted">'.JText::_('ORDER_ROW_DATE').'</span><span class="fnum">'.Jhtml::date($this->element->order_created, 'D ، d M Y').'</span><br>';
                } else {
                    echo '<span class="uk-text-muted">'.JText::_('ORDER_ROW_DATE').'</span>'.hikashop_getDate($this->element->order_invoice_created, '%d %B %Y').'<br>';
                }
                ?>
                <!-- EO DATE -->
                ...
                if(!empty($this->shipping)) {
                    echo '<span class="uk-text-muted">'.JText::_('HIKASHOP_SHIPPING_METHOD').'</span>';
                ...
                if(!empty($this->payment)) {
                    echo '<span class="uk-text-muted">'.JText::_('HIKASHOP_PAYMENT_METHOD') . '</span>' . $this->payment->getName($this->order->order_payment_method, $this->order->order_payment_id).'<br>';
                }
                if($order_weight) {
                    echo '<span class="uk-text-muted">'.JText::_('HIKASHOP_TOTAL_ORDER_WEIGHT') . '</span>' . rtrim(rtrim($this->order->order_weight,'0'),',.').' '.JText::_($this->order->order_weight_unit).'<br>';
                }
                ?>
        ...
        <?php if(!empty($this->element->order_shipping_id) && !empty($this->element->shipping_address)) { ?>
            <div class="uk-width-1-1 uk-width-expand@m">
                <div class="adminform" id="htmlfieldset_shipping">
                    <h3 class="uk-text-primary uk-margin-bottom sectionTitle font"><?php echo JText::_('HIKASHOP_SHIPPING_ADDRESS'); ?></h3>
                    <?php
                    $override = false;
        ...
        <div class="uk-width-1-1">
            <h3 class="uk-text-primary uk-margin-bottom sectionTitle font"><?php echo JText::_('PRODUCT_LIST'); ?></h3>
            <table class="uk-table uk-table-middle uk-table-striped uk-margin-remove uk-text-zero">
                <thead>
                <tr>
                    ...
                    <!-- EO WEIGHT HEADER -->
                    <!-- PRICE HEADER -->
                    <th class="hikashop_order_item_price_title title uk-text-center"><?php
                        echo JText::_('CART_PRODUCT_UNIT_PRICE');
                        ?></th>
                    <!-- EO PRICE HEADER -->
                    <!-- QUANTITY HEADER -->
                    <th class="hikashop_order_item_quantity_title title uk-text-center"><?php
                    ...
                    <!-- TOTAL PRICE HEADER -->
                    <th class="hikashop_order_item_total_title title uk-text-center"><?php
                ...
                foreach($this->order->products as $product) {
                    ...
                    <tr class="row<?php echo $k;?>">
                        ...
                        <td data-title="<?php echo JText::_('HIKA_IMAGE'); ?>" class="hikashop_order_item_image_value uk-table-shrink">
                            ...
                            if($img->success) {
                                echo '<span class="uk-card uk-card-default uk-border-rounded uk-box-shadow-small uk-display-inline-block uk-padding-small"><img class="hikashop_order_item_image uk-preserve-width" title="'.$this->escape(@$product->images[0]->file_description).'" alt="'.$this->escape(@$product->images[0]->file_name).'" src="'.$img->url.'"/></span>';
                            }
                            ?>
                        </td>
                        <!-- EO IMAGE -->
        ...
        <div class="uk-width-1-1">
            <div>
                <h3 class="uk-text-primary uk-margin-bottom sectionTitle font"><?php echo JText::sprintf('ORDER_SUMMERY'); ?></h3>

                <!-- SUBTOTAL -->
                <div class="uk-grid-small" data-uk-grid>
                    <div class="uk-width-expand uk-leader uk-first-column" data-uk-leader>
                        <span class="uk-text-small uk-text-secondary f500 font"><?php echo JText::_( 'SUBTOTAL' ); ?></span>
                    </div>
                    <div>
                ...
                <?php if(!empty($this->order->order_shipping_method)) { ?>
                    <div class="uk-grid-small" data-uk-grid>
                        <div class="uk-width-expand uk-leader uk-first-column" data-uk-leader>
                            <span class="uk-text-small uk-text-secondary f500 font"><?php echo JText::_( 'HIKASHOP_SHIPPING_COST' ); ?></span>
                        </div>
                        <div>
                    <span class="hikashop_checkout_cart_subtotal uk-text-small uk-text-primary f600 font fnum">
                        <?php
                ...
                <div class="uk-grid-small" data-uk-grid>
                    <div class="uk-width-expand uk-leader uk-first-column" data-uk-leader>
                        <span class="uk-text-small uk-text-secondary f500 font"><?php echo JText::_( 'HIKASHOP_TOTAL' ); ?></span>
                    </div>
                    <div>
                <span class="hikashop_checkout_cart_subtotal uk-text-small uk-text-primary f600 font fnum">
